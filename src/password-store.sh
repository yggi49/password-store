...
cmd_usage() {
	...
	Usage:
	    ...
	    $PROGRAM [show] [--clip[=line-number],-c[line-number]] [--meta,-m] pass-name
...
cmd_show() {
	local opts selected_line clip=0 qrcode=0 meta=0
	opts="$($GETOPT -o q::c::m -l qrcode::,clip::,meta -n "$PROGRAM" -- "$@")"
	local err=$?
	eval set -- "$opts"
	while true; do case $1 in
	...
	[[ $err -ne 0 || ( $qrcode -eq 1 && $clip -eq 1 ) ]] && die "Usage: $PROGRAM $COMMAND [--clip[=line-number],-c[line-number]] [--qrcode[=line-number],-q[line-number]] [--meta,-m] [pass-name]"
	...
	if [[ -f $passfile ]]; then
		if [[ $clip -eq 0 && $qrcode -eq 0 ]]; then
			if [[ $meta -eq 1 ]]; then
				pass="$($GPG -d "${GPG_OPTS[@]}" "$passfile" | tail -n +2 | $BASE64)" || exit $?
			else
